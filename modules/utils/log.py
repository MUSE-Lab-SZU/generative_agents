...
class IOLogger(object):
    ...
    def _safe_emit(self, color, message):
        printer = self._get_printer(color)
        try:
            printer(message)
            ...
            printer(safe_message)

    def info(self, msg):
        if self._level <= logging.INFO:
            self._safe_emit("green", "[INFO]{}: {}".format(self._prefix(), msg))

    def debug(self, msg):
        if self._level <= logging.DEBUG:
            self._safe_emit("green", "[DEBUG]{}: {}".format(self._prefix(), msg))

    def warning(self, msg):
        if self._level >= logging.WARN:
            self._safe_emit("yellow", "[WARNING]{}: {}".format(self._prefix(), msg))

    def error(self, msg):
        self._safe_emit("red", "[ERROR]{}: {}".format(self._prefix(), msg))
        raise Exception(msg)
