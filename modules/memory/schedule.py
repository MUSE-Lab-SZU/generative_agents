...
class Schedule:
    ...
    def insert_hard_plan(self, describe, start, duration, source="intervention", meta=None):
        """Insert a hard plan block and keep timeline continuous."""
        start = max(0, min(int(start), 24 * 60 - 1))
        ...
        rebuilt = []
        for plan in self.daily_schedule:
            p_start = int(plan["start"])
            p_end = int(plan["start"] + plan["duration"])
        ...
        rebuilt.append(forced)
        rebuilt = sorted(rebuilt, key=lambda x: x["start"])
        for idx, plan in enumerate(rebuilt):
            plan["idx"] = idx
        self.daily_schedule = rebuilt
        return forced
