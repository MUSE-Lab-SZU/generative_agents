...
class ComplaintStage:
    ...
    topic_id: str = ""
    source_kind: str = "legacy_unknown"
...
class ComplaintGraphManager:
    ...
    def __init__(
        ...
        domain_state_enabled: bool = False,
    ):
        ...
        self._initialization_config_path = config.get("_config_path") or "inline_config"
        self._initialization_pointer_prefix = (
            "/complaint_graph" if "complaint_graph" in config else ""
        )
        self.pipeline_version = graph_config.get("pipeline_version", "v3")
        if self.pipeline_version not in {"v3", "foundation_legacy"}:
            raise ValueError("INVALID_PIPELINE_VERSION")
        if self.pipeline_version == "v3" and domain_state_enabled:
            raise ValueError("V3_DOMAIN_STATE_INCOMPATIBLE")
        if self.pipeline_version == "foundation_legacy":
            self.TRANSITION_CANDIDATE_COUNT = 3
        ...
        self.reset()
        self._initialize_v3(graph_config)
        if not graph_config.get("_restoring_v3"):
            for node in self.stage_catalog.values():
    ...
    def _initialize_v3(self, config):
        self.graph_state_version = 0
        self.case_id = new_id("case")
        self.origin_namespace = new_id("origin")
        self.topic_registry = copy.deepcopy(config.get("topic_registry", {}))
        initialization_config = config.get("_initialization_config", config)
        self.frozen_initialization = {
            "config": copy.deepcopy(initialization_config),
            "config_fingerprint": digest(initialization_config),
        }
        for key in self.V3_FIELDS[5:]:
            setattr(self, key, {})
        self._legacy_errors = []
        for node in self.stage_catalog.values():
            topic = node.get("topic_id") or "topic_" + digest(node["id"])[:24]
            node["topic_id"] = topic
            self.topic_registry.setdefault(
                topic, {"neutral_name": "初始主诉", "origin_node_id": node["id"]}
            )
            if config.get("_restoring_v3"):
                # Restored refs are validated after the saved ledger is loaded.
                continue
            raw_stages = initialization_config.get(
                "stages", initialization_config.get("stage_catalog", [])
            )
            raw_stages = list(raw_stages.values()) if isinstance(raw_stages, dict) else raw_stages
            raw = next(
                (
                    x
                    for x in raw_stages
                    if isinstance(x, dict)
                    and (x.get("id") or self._make_stage_id(x.get("label", "未命名主诉节点")))
                    == node["id"]
                ),
                {},
            )
            if (
                not raw
                and node["id"] == self.initial_stage_id
                and not config.get("_legacy_restore")
            ):
                raw = copy.deepcopy(node)
            claims = copy.deepcopy(raw.get("accepted_claims", []))
            if not claims and raw and node["id"] == self.initial_stage_id:
                claims = [{"kind": "unknown", "text": raw.get("summary", node["summary"])}]
            if claims:
                ref = new_id("config")
                self.evidence_ledger[ref] = dict(
                    evidence_id=ref,
                    record_kind="persona_fragment",
                    case_id=self.case_id,
                    config_fingerprint=digest(initialization_config),
                    config_path=self._initialization_config_path,
                    json_pointer=self._initialization_pointer_prefix
                    + "/stages/"
                    + str(raw_stages.index(raw) if raw in raw_stages else 0),
                    raw_fragment=copy.deepcopy(raw),
                    captured_at=self._now().isoformat(),
                )
                for c in claims:
                    c.setdefault("claim_id", new_id("claim"))
                    c.setdefault("revision", 1)
                    c.setdefault("topic_id", topic)
                    c.setdefault("subject", "patient")
                    c.setdefault("assertion_status", "unknown")
                    c.setdefault("actuality", "unknown")
                    c.setdefault(
                        "time",
                        dict(
                            reported_time_text="",
                    ...
                    c.update(acceptance_basis="configured", evidence_refs=[ref])
                    c.setdefault("accepted_at", self._now().isoformat())
                    c.setdefault("accepted_version", 0)
                    c.setdefault("supersedes", None)
                node.update(
                    accepted_claims=claims,
                    source_kind="persona_initialization",
                    migration_status=(
                        "legacy_config_opaque"
                        if any(c["kind"] == "unknown" for c in claims)
                        else "native_v3"
                    ),
                )
    ...
    def _restore_v3(self, payload):
        ...
        if self.pipeline_version == "phase1_legacy_transition":
            self.pipeline_version = "foundation_legacy"
        if self.pipeline_version not in {'v3','foundation_legacy'}:
            raise ValueError('INVALID_PIPELINE_VERSION')
        self.stage_catalog = copy.deepcopy(payload["stage_catalog_snapshot"])
        self._core_belief = payload["core_belief"]
        self._root_complaint_anchor = payload["root_complaint_anchor"]
        required = {f.name for f in fields(ComplaintStage)}
        for key, node in self.stage_catalog.items():
            if not isinstance(node, dict) or not required <= node.keys() or node["id"] != key:
                raise ValueError("INVALID_V3_NODE_RECORD")
            self._sanitize_stage(node)  # Validate enums; do not truncate saved display content.
            if node["core_belief"] != self._core_belief:
                raise ValueError("CORE_BELIEF_MIRROR_CONFLICT")
        for key in self.V3_FIELDS:
        ...
        if (
            not path
            or any(x not in self.stage_catalog for x in path)
            or not 0 <= index < len(path)
            or len(set(path[: index + 1])) != index + 1
        ):
            raise ValueError("INVALID_COMMITTED_PATH")
        if path[index] != payload["current_stage_id"]:
            raise ValueError("INVALID_CURRENT_NODE")
        self.planned_graph, self.stage_index = list(path), index
        for key, row in self.evidence_ledger.items():
            if row.get("evidence_id") != key or row.get("case_id") != self.case_id:
                raise ValueError("INVALID_LEDGER_RECORD")
            if row.get("record_kind") == "message":
                resolve_messages(self.evidence_ledger, [key], self.case_id, patient_only=False)
                session = self.session_records.get(row.get("session_instance_id"), {})
                if key not in session.get("message_refs", []):
                    raise ValueError("MISSING_MESSAGE_SESSION_REF")
                snapshot = row.get("generation_snapshot_ref")
                if snapshot and (
                    snapshot not in self.generation_snapshots
                    or self.generation_snapshots[snapshot].get("generation_attempt_id")
                    != row.get("generation_attempt_id")
                ):
                    raise ValueError("INVALID_GENERATION_SNAPSHOT")
        for session_id, session in self.session_records.items():
            if len(set(session["message_refs"])) != len(session["message_refs"]):
                raise ValueError("DUPLICATE_SESSION_REF")
            for ordinal, row in enumerate(
                resolve_messages(
                    self.evidence_ledger, session["message_refs"], self.case_id, patient_only=False
                ),
                start=1,
            ):
                if row.get("ordinal") != ordinal:
                    raise ValueError("INVALID_MESSAGE_ORDINAL")
                if row["session_instance_id"] != session_id:
                    raise ValueError("INVALID_SESSION_REF")
        revisions = {}
        for node in self.stage_catalog.values():
            self._validate_snapshot(node)
            for claim in node["accepted_claims"]:
                key = (claim["claim_id"], claim["revision"])
                if key in revisions and revisions[key] != claim:
                    raise ValueError("CLAIM_REVISION_CONFLICT")
                revisions[key] = claim
        for eid, event in self.pending_events.items():
            if (
                event.get("metadata", {}).get("event_id") != eid
                or event.get("base_node_id") not in self.stage_catalog
            ):
                raise ValueError("INVALID_PENDING_EVENT")
            if eid in self.processed_event_results:
                raise ValueError("PENDING_EVENT_ALREADY_PROCESSED")
            validate_event_sources(self, event)
        if set(self.processed_event_results) != set(self.transition_traces):
            raise ValueError("MISSING_EVENT_RESULT_OR_TRACE")
        for eid, trace in self.transition_traces.items():
            resolve_messages(self.evidence_ledger, trace.get("accepted_refs", []), self.case_id)
            if (
                trace.get("event_id") != eid
                or trace.get("final_result") != self.processed_event_results[eid]
            ):
                raise ValueError("EVENT_RESULT_TRACE_CONFLICT")
            if trace.get("pipeline_version") == "v3":
                for o in trace.get("extraction", {}).get("normalized", {}).get("observations", []):
                    gate_message_refs(self, o["supporting_message_ids"], trace["accepted_refs"])
                    if o["event_id"] != eid:
                        raise ValueError("OBSERVATION_EVENT_CONFLICT")
                validation = trace.get("validation", {})
                if validation.get("status") == "ok":
                    v = validation["normalized"]
                    p = trace["proposal"]["normalized"]
                    if (v["input_hash"] != digest(validation["input"]) or
                        v["proposal_id"] != p["proposal_id"] or validation["input"]["proposal"] != p):
                        raise ValueError("VALIDATION_BINDING_CONFLICT")
                for ref in trace.get("base_claim_refs", []):
        ...
        for ref, snapshot in self.generation_snapshots.items():
            if snapshot.get("generation_snapshot_ref") != ref:
                raise ValueError("GENERATION_SNAPSHOT_ID_CONFLICT")
            baseline = self.resolve_active_claims(snapshot.get("stage_id"))["active_claims"]
            allowed = {(c["claim_id"], c["revision"]) for c in baseline}
            if any(
                (c["claim_id"], c["revision"]) not in allowed
                for c in snapshot.get("claim_refs", [])
            ):
                raise ValueError("DANGLING_GENERATION_CLAIM_REF")
    ...
    def _validate_snapshot(self, node):
        seen = set()
        for raw in node["accepted_claims"]:
            c = validate_claim(raw)
            if c["claim_id"] in seen:
                raise ValueError("MULTIPLE_CLAIM_HEADS")
            seen.add(c["claim_id"])
            if c["topic_id"] not in self.topic_registry:
                raise ValueError("INVALID_CLAIM_TOPIC")
            for ref in c["evidence_refs"]:
                row = self.evidence_ledger.get(ref, {})
                if row.get("case_id") != self.case_id:
                    raise ValueError("DANGLING_CLAIM_EVIDENCE")
                if c["acceptance_basis"] == "configured":
                    if row.get("record_kind") != "persona_fragment":
                        raise ValueError("INVALID_CONFIG_EVIDENCE")
                else:
                    resolve_messages(self.evidence_ledger, [ref], self.case_id)
        if node["topic_id"] not in self.topic_registry:
            raise ValueError("INVALID_NODE_TOPIC")
        if sorted(node["evidence_refs"]) != sorted(
            {r for c in node["accepted_claims"] for r in c["evidence_refs"]}
        ):
            raise ValueError("INVALID_NODE_EVIDENCE_UNION")

    def resolve_active_claims(self, base_node_id=None):
        target = base_node_id or self.get_current_stage_id()
        committed = self.planned_graph[: self.stage_index + 1]
        if target not in committed:
            raise ValueError("UNCOMMITTED_BASE_NODE")
        heads, revisions = {}, {}
        for node_id in committed[: committed.index(target) + 1]:
            node = self.stage_catalog[node_id]
            self._validate_snapshot(node)
            next_heads = {}
            for c in node["accepted_claims"]:
                cid, rev = c["claim_id"], c["revision"]
                key = (cid, rev)
                if key in revisions and revisions[key] != c:
                    raise ValueError("CLAIM_REVISION_CONFLICT")
                old = heads.get(cid)
                if old and rev != old["revision"]:
                    if (
                        rev != old["revision"] + 1
                        or c.get("supersedes") != {"claim_id": cid, "revision": old["revision"]}
                        or any(c[k] != old[k] for k in ("subject", "kind"))
                        or old["kind"] == "unknown"
                    ):
                        raise ValueError("BROKEN_SUPERSEDES_CHAIN")
                elif not old and (rev != 1 or c.get("supersedes") is not None):
                    raise ValueError("BROKEN_SUPERSEDES_CHAIN")
                revisions[key] = copy.deepcopy(c)
                next_heads[cid] = copy.deepcopy(c)
            if set(heads) - set(next_heads):
                raise ValueError("CLAIM_DELETION_NOT_ALLOWED")
            heads = next_heads
        return {
            "active_claims": [c for c in heads.values() if c["kind"] != "unknown"],
            "initialization_background": [c for c in heads.values() if c["kind"] == "unknown"],
        }

    def resolve_claim_revision(self, claim_id, revision, purpose="current"):
        active = self.resolve_active_claims()
        for c in active["active_claims"]:
            if (c["claim_id"], c["revision"]) == (claim_id, revision):
                return c
        if purpose == "history_audit":
            for nid in self.planned_graph[: self.stage_index + 1]:
                for c in self.stage_catalog[nid]["accepted_claims"]:
                    if (c["claim_id"], c["revision"]) == (claim_id, revision):
                        return copy.deepcopy(c)
        raise ValueError("NON_ACTIVE_CLAIM_REVISION")

    def update_display(self, node_id, **changes):
        with self._commit_lock:
            return self._update_display(node_id, **changes)

    def _update_display(self, node_id, **changes):
    ...
    def start_session(self, session_id=None):
        sid = check_id(session_id or new_id("session"))
        self.session_records.setdefault(
            sid, {"message_refs": [], "origin_namespace": self.origin_namespace}
        )
        return sid

    def register_accepted_message(
        self,
        text,
        speaker_id,
        session_instance_id,
        ...
        **metadata,
        ...
        if session_instance_id not in self.session_records:
            raise ValueError("INVALID_SESSION")
        sid = session_instance_id
        mid = message_id or new_id("message")
        if mid in self.evidence_ledger:
            old = self.evidence_ledger[mid]
            if any(
                old.get(k) != v
                for k, v in dict(
                    text=text,
            ...
            for key in ("generation_attempt_id", "generation_snapshot_ref", "meeting_id"):
                if key in metadata and metadata[key] != old.get(key):
                    raise ValueError("MESSAGE_ID_CONFLICT")
            resolve_messages(self.evidence_ledger, [mid], self.case_id, patient_only=False)
            return mid
        record = dict(
            message_id=mid,
            case_id=self.case_id,
        ...
        snapshot = record["generation_snapshot_ref"]
        if snapshot and snapshot not in self.generation_snapshots:
            raise ValueError("INVALID_GENERATION_SNAPSHOT")
        register_accepted_message(self.evidence_ledger, record)
        self.session_records[sid]["message_refs"].append(mid)
        return mid
    ...
    def reset(self) -> None:
        if hasattr(self, "evidence_ledger"):
            fresh = type(self)(
                copy.deepcopy(self.frozen_initialization["config"]),
                self._now_provider,
    ...
    def _commit_legacy_turn(self, evaluation: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not evaluation.get("event_id"):
            raise ValueError("ACCEPTED_EVENT_REQUIRED")
    ...
    def to_dict(self) -> Dict[str, Any]:
        with self._commit_lock:
            return json_safe(self._serialize_state())
    ...
    def from_dict(
        ...
        if payload.get("graph_schema_version") not in {None, 1, 2, 3}:
        ...
        if payload.get("graph_schema_version") == 3:
            snapshot = payload.get("stage_catalog_snapshot")
            if not isinstance(snapshot, dict) or not snapshot:
                raise ValueError("MISSING_CATALOG_SNAPSHOT")
            stage_catalog = list(snapshot.values())
        initial_stage_id = str(
            payload.get(
                "initial_stage_id",
                payload.get(
        ...
        cfg["pipeline_version"] = payload.get("pipeline_version", "foundation_legacy")
        if cfg["pipeline_version"] == "phase1_legacy_transition":
            cfg["pipeline_version"] = "foundation_legacy"
        cfg["_restoring_v3"] = payload.get("graph_schema_version") == 3
        if not cfg["_restoring_v3"]:
            cfg["_legacy_restore"] = True
            cfg["_initialization_config"] = copy.deepcopy(payload_cfg or base_cfg)
        ...
        if payload.get("graph_schema_version") == 3:
            manager._restore_v3(payload)
        else:
            previous_claims = []
            for nid in manager.planned_graph[: manager.stage_index + 1]:
                node = manager.stage_catalog[nid]
                if not node["accepted_claims"]:
                    node["accepted_claims"] = copy.deepcopy(previous_claims)
                    node["evidence_refs"] = sorted(
                        {r for c in previous_claims for r in c["evidence_refs"]}
                    )
                previous_claims = node["accepted_claims"]
            manager.frozen_initialization["legacy_config_provenance"] = (
                "inline_config" if payload_cfg else "current_file_fallback"
            )
        if payload.get("graph_schema_version") != 3:
            manager._link_graph_edges(manager.planned_graph)
            manager._ensure_future_window()
        manager.resolve_active_claims()
        return manager
    ...
    def _build_transition_change_prompt(
        ...
        return render_prompt(
            "depression/graph_transition_change_legacy",
    ...
    def _build_transition_prompt(
        ...
        return render_prompt(
            "depression/graph_transition_legacy",
            {"payload_json": json.dumps(payload, ensure_ascii=False)},
        )
    ...
    def _normalize_branch_seed_result(
        ...
        for item in self._to_list(raw_items):
            if not isinstance(item, dict):
                ...
                continue
            if any(
                k in item for k in ("accepted_claims", "topic_id", "source_kind", "evidence_refs")
            ):
                self._legacy_errors.append("LEGACY_PROTECTED_FIELD_IGNORED")
    ...
    def _normalize_branch_detail(
        ...
        if any(
            key in detail for key in ("accepted_claims", "topic_id", "source_kind", "evidence_refs")
        ):
            self._legacy_errors.append("LEGACY_PROTECTED_FIELD_IGNORED")
    ...
    def _build_graph_prompt(
        ...
        payload_json = json.dumps(payload, ensure_ascii=False)
        return render_prompt(
            "depression/graph_planner_legacy",
    ...
    def _sanitize_stage(self, raw: Any, source: str = "config") -> Dict[str, Any]:
        ...
        if len(stage_id) > 80:
            raise ValueError("INVALID_NODE_ID_LENGTH")
        ...
        if result["node_kind"] not in CLAIM_KINDS | {"unknown", "mixed"}:
            raise ValueError("INVALID_NODE_KIND")
        ...
        if result["migration_status"] not in {
            "native_v3",
            "legacy_config_opaque",
            "legacy_unverified",
            "legacy_transition_unverified",
        }:
            raise ValueError("INVALID_MIGRATION_STATUS")
        if (
            result["schema_version"] != 3
            or type(result["content_revision"]) is not int
            or result["content_revision"] < 1
        ):
