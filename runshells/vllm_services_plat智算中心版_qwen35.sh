...
QWEN_MODEL_DIR="${QWEN_MODEL_DIR:-$MODEL_ROOT/Qwen3.5-9B}"
...
# Qwen3.5-9B weights are BF16; 4090-class cards support BF16 natively.
QWEN_DTYPE="${QWEN_DTYPE:-bfloat16}"
EMBED_DTYPE="${EMBED_DTYPE:-half}"
# Qwen3.5-9B is a multimodal checkpoint.  The simulation is text-only, so
# launch only its language component.  These conservative defaults match the
# single-24GB-GPU quick probe and leave room for the KV-cache startup profile.
...
start_service() {
    ...
    if [[ "$service_name" == Qwen3* ]]; then
        # `/nothink` is a useful per-request hint, but Qwen3.5 can still
        # reason when callers omit it.  Keep text-simulation responses short
        # and deterministic by disabling its chat-template thinking mode at
        # the server as well.  These are separate array items so the JSON is
        # passed to vLLM unchanged.
        extra_args+=(
...
print_status() {
    ...
    echo "vLLM 服务状态"
    echo "  Qwen3.5 文本模式: language-model-only, thinking=disabled, max_len=$QWEN_MAX_MODEL_LEN, max_seqs=$QWEN_MAX_NUM_SEQS, max_batched_tokens=$QWEN_MAX_NUM_BATCHED_TOKENS"
...
多卡示例（1/2/8 卡单服务模式需先清空默认服务池）：
  ...
  4 卡（3 个 Qwen、3 个 BGE）:
    QWEN_SERVICE_GPUS=0,1,2 QWEN_PORTS=18000,18002,18003 \\
    EMBED_SERVICE_GPUS=3,3,3 EMBED_PORTS=18001,18004,18005 \\
    bash runshells/vllm_services_plat智算中心版_qwen35.sh start
  8 卡: QWEN_GPUS=0,1,2,3,4,5,6 EMBED_GPUS=7 bash runshells/vllm_services_plat智算中心版_qwen35.sh start
    ...
    start)
        ...
        for index in "${!QWEN_SERVICE_GPU_LIST[@]}"; do
            ...
            start_service "Qwen3[$index]" "$QWEN_MODEL_DIR" "$gpu" "${QWEN_SERVICE_PORT_LIST[$index]}" "$QWEN_NAME" \
                "$QWEN_DTYPE" "$QWEN_GPU_MEMORY_UTILIZATION" "$QWEN_MAX_MODEL_LEN" "$(resolve_tensor_parallel_size "$QWEN_TENSOR_PARALLEL_SIZE" "$gpu")" "--language-model-only" \
                "$(pool_pid_file qwen3 "$index")" "$(pool_log_file qwen3 "$index")" "$QWEN_MAX_NUM_SEQS" "$QWEN_MAX_NUM_BATCHED_TOKENS"
        done
        for index in "${!EMBED_SERVICE_GPU_LIST[@]}"; do
            gpu="${EMBED_SERVICE_GPU_LIST[$index]}"
