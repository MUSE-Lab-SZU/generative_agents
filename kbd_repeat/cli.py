"""Command-line orchestration for KBD repeat-summary reporting."""
...
from .plots import render_appendix, render_lines_only, render_presentation
...
OUT_DIR = PROJECT_ROOT / "docs" / "kbd_repeat_charts"
RECURSIVE = False
TITLE_PREFIX = "KBD repeat evaluation"
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--report-mode",
        choices=["lines-only", "presentation", "appendix", "all"],
        default="all",
        help="Generate lines-only samples, presentation charts, backward-compatible appendix charts, or all charts (default: all).",
    )
    parser.add_argument(
        "--error-bar",
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(f"Detected repeats: {', '.join(all_repeats)}")
