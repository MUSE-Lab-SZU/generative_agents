...
CACHE_ROOT = Path(tempfile.gettempdir()) / "kbd_repeat_chart_cache"
...
def save_figure(fig: Any, png_path: Path) -> Path:
    """Save every chart as 300-DPI PNG plus SVG and PDF without deleting old files."""
    png_path.parent.mkdir(parents=True, exist_ok=True)
    for suffix in (".png", ".svg", ".pdf"):
        target = png_path.with_suffix(suffix)
...
def _footer(
    ...
    endpoint_rule = "endpoint: POST > NOW > last available"
    outer_counts = sorted(
...
def plot_endpoint_delta_with_ci(
    ...
    for row, scale in enumerate(scales):
        for column, severity in enumerate(severities):
            ...
            ax.set_ylabel("Endpoint − baseline")
            ax.grid(axis="y", alpha=0.22)
    fig.suptitle(_wrapped(f"{title_prefix}: endpoint change with 95% difference CI"), fontsize=13)
    ns = sorted({record.expected_repeats for record in records})
    fig.text(
        0.5,
        -0.025,
        f"n={','.join(map(str, ns))} repeats/snapshot; endpoint: POST > NOW > last available. "
    ...
    for scale in scales:
        for metric, metric_label in MEASUREMENT_METRICS.items():
            ...
            fig.text(
                ...
                "POST > NOW > last available. m=modal confidence; rows=independent outer experiments.",
                ha="center",
                fontsize=8,
            )
